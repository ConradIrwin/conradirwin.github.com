...
computer. I think this is one of the massively underestimated features of new-school
...
To be honest point one is both the most obvious advantage and the least useful — most editors
have undo features, so this is only game changing if your machine crashes a lot (maybe
essential for Kernel developers, but not really for me).
...
committing (with `git commit -v` so I can review my own diff) that I've just rewritten a
lot of logic from the main sign-up form... This sets my refactor alert off, and so after
...
into a shareable form; and then add a password reset dialog that uses the new sharable
code.
